function [weights] = STDP_label_5_5(layers,learning_layer,STDP_index,weights,network_struct,deta_STDP_minus_r ,deta_STDP_plus_r,deta_STDP_minus_p,deta_STDP_plus_p,label)
             ...
             %����------------------------------------------------------------------------------------------
                        ...
                        for i=1:H
                            for j=1:W
                                if local_K_STDP(i,j,k)==0
                                    ...
                                    if local_K_STDP(i,j,k)-t>=0      %LTD����
                                        dw=-deta_STDP_minus_r(local_K_STDP(i,j,k)-t+1)*w(i,j,k,sk); 
                                    elseif local_K_STDP(i,j,k)-t<0   %LTP����  
                    ...
                    for k=1:D
                        for i=1:H
                            for j=1:W
                                if local_K_STDP(i,j,k)==0
                                    dw=deta_STDP_plus_p(1)*w(i,j,k,sk);
                                else
                                    if local_K_STDP(i,j,k)-t<=0  %LTD �ͷ�
                                        dw=-deta_STDP_minus_p(t-local_K_STDP(i,j,k)+1)*w(i,j,k,sk);
                                    elseif local_K_STDP(i,j,k)-t>0 %LTP �ͷ�
                                        dw=deta_STDP_plus_p(local_K_STDP(i,j,k)-t)*(1-w(i,j,k,sk));
                                    end
                                end
                                w(i,j,k,sk)=w(i,j,k,sk)+dw;
                                if w(i,j,k,sk)>0.999999
                        ...
                        for i=1:H
                            for j=1:W
                                if local_K_STDP(i,j,k)==0
                                    ...
                                    if local_K_STDP(i,j,k)-t<=0%�ͷ�LTD 
                                        dw=-deta_STDP_minus_p(t-local_K_STDP(i,j,k)+1)*w(i,j,k,sk);%�ͷ�
                                    elseif local_K_STDP(i,j,k)-t>0%�ͷ�LTP 
                                        dw=deta_STDP_plus_p(local_K_STDP(i,j,k)-t)*(1-w(i,j,k,sk));
                                    end
                                end
                                w(i,j,k,sk)=w(i,j,k,sk)+dw;
